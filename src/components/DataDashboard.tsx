        ...
        <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-md z-[1000] flex flex-col p-6 font-mono text-slate-200">
            ...
            <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
                <h2 className="text-2xl font-bold text-orange-500 flex items-center gap-2">
                    <Microscope /> Forskningscenter <span className="text-sm bg-orange-900/30 text-orange-300 px-2 py-0.5 rounded border border-orange-500/30">DATABRICKS CONNECTED</span>
                </h2>
            ...
            <div className="grid grid-cols-3 gap-6 flex-1 overflow-hidden">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-800/30 p-3 rounded border border-slate-700">
                            <div className="text-[12px] text-slate-400 uppercase font-bold">Active Nodes</div>
                            <div className="text-lg font-black text-green-400 mt-1">8 / 8</div>
                        </div>
                        <div className="bg-slate-800/30 p-3 rounded border border-slate-700">
                            <div className="text-[12px] text-slate-400 uppercase font-bold">Memory</div>
                            <div className="text-lg font-black text-yellow-400 mt-1">64%</div>
                        </div>
                        <div className="bg-slate-800/30 p-3 rounded border border-slate-700">
                            <div className="text-[12px] text-slate-400 uppercase font-bold">Ingest Rate</div>
                            <div className="text-lg font-black text-blue-400 mt-1">2.4 GB/s</div>
                        </div>
                        <div className="bg-slate-800/30 p-3 rounded border border-slate-700">
                            <div className="text-[12px] text-slate-400 uppercase font-bold">Latency</div>
                    ...
                    <div className="h-32 bg-slate-950 rounded border border-slate-800 relative overflow-hidden p-2">
                        ...
                        <div className="absolute top-2 left-2 text-[12px] text-slate-400 uppercase font-bold">Cluster Load (1m)</div>
                    </div>

                    {/* AI Engine Status Card */}
                    <div className="mt-2 bg-slate-950 border border-slate-850 p-4 rounded-xl flex flex-col gap-2">
                        <div className="text-sm font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                            <Cpu size={14} className="text-orange-400" /> Model Insights
                        </div>
                        {currentStrategy === 'REACTIVE' && (
                            <div className="text-[12px] text-rose-400/80 leading-relaxed">
                        ...
                        {currentStrategy === 'PREVENTIVE' && (
                            <div className="text-[12px] text-amber-400/80 leading-relaxed">
                        ...
                        {currentStrategy === 'CONDITIONAL' && (
                            <div className="text-[12px] text-orange-400/80 leading-relaxed">
                    ...
                    {/* Dataanalyse & Audits Card */}
                    <div className="bg-gradient-to-r from-orange-950/20 to-amber-950/20 border border-orange-500/20 p-4 rounded-xl flex flex-col gap-3">
                        <div className="text-sm font-bold text-slate-350 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-800 pb-1">
                            📊 Dataanalyse & Audits
                        </div>
                        {isDataAuditActive ? (
                            <div className="flex flex-col gap-1.5 text-[12px] text-slate-400">
                                <div className="flex justify-between font-bold">
                                    <span>Kører analyse...</span>
                                    <span className="text-orange-400 font-mono">~{Math.ceil(dataAuditTimer)}s</span>
                                </div>
                                <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-850">
                                    <div 
                                       className="bg-orange-500 h-full animate-pulse" 
                                       style={{ width: `${(1.0 - (dataAuditTimer / 900)) * 100}%` }}
                                    ></div>
                                </div>
                            </div>
                        ) : (
                            <div className="flex flex-col gap-2">
                                <p className="text-[12px] text-slate-400 leading-normal">
                                    Kør en dataanalyse af linjedriften for at modtage en forskningsbevilling på $500. Kræver Dataanalytikere for optimal hastighed.
                                </p>
                                <button
                                    onClick={() => onStartDataAudit && onStartDataAudit()}
                                    className="w-full py-1.5 rounded text-[12px] font-bold bg-orange-600 hover:bg-orange-500 text-white transition-all active:scale-95 shadow-md"
                                >
                                    Start Data-Audit
                                </button>
                            </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col overflow-hidden">
                    <h3 className="text-sm font-semibold text-slate-400 flex items-center gap-2 border-b border-slate-800 pb-2 mb-1 cursor-help" title="Bronze Layer: Rå og ubehandlede sensordata direkte fra metrotogene i realtid.">
                        <Activity size={16} className="text-blue-400" /> Telemetry (Bronze Layer) <span className="text-[12px] text-slate-400 font-normal">(Info ℹ️)</span>
                    </h3>
                    <p className="text-[12px] text-slate-400 mb-2">Rå realtids sensordata direkte fra togene.</p>
                    <div className="flex-1 overflow-y-auto font-mono text-[12px] space-y-1 custom-scrollbar bg-black/50 p-3 rounded border border-slate-950">
                        {logs.map(log => (
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col overflow-hidden">
                    <h3 className="text-sm font-semibold text-slate-400 flex items-center gap-2 border-b border-slate-800 pb-2 mb-1 cursor-help" title="Silver Layer: Udvundne og rensede sensormønstre. Gold Layer: Trænede ML-modeller, der forudsiger nedbrud (RUL) og foreslår forebyggende vedligeholdelse.">
                        <Brain size={16} className="text-purple-400" /> AI Predictions (Gold Layer) <span className="text-[12px] text-slate-400 font-normal">(Info ℹ️)</span>
                    </h3>
                    <p className="text-[12px] text-slate-400 mb-2">Silver: Filtrerede fejl. Gold: ML-forudsigelser og RUL.</p>
                    ...
                    <div className="bg-gradient-to-r from-purple-950/40 to-indigo-950/40 border border-purple-500/25 p-3 rounded-xl flex flex-col gap-1 mb-3 shrink-0">
                        ...
                        <div className="flex justify-between items-baseline">
                            <span className="text-sm text-slate-400">Penge sparet via data:</span>
                            <span className="text-lg font-black text-emerald-400 font-mono">${dataLakeSavings.toLocaleString()}</span>
                        </div>
                    </div>

                    {/* Predictions list */}
                    <div className="flex-1 overflow-y-auto font-mono text-[12px] space-y-3 custom-scrollbar bg-black/50 p-3 rounded border border-slate-950 mb-3">
                                ...
                                return (
                                    <div key={anom.id} className={`p-3 rounded-xl border flex flex-col gap-2 ${
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className={`text-[12px] font-bold px-1.5 py-0.5 rounded uppercase ${
                                                anom.failed 
                                        ...
                                        {/* Predictions details */}
                                        {!anom.failed && (
                                            <div className="bg-slate-950/50 p-2 rounded border border-slate-900 text-[12px] flex flex-col gap-1 text-slate-400">
                                        ...
                                        {!anom.stewardDeployed && (
                                            <button
                                                ...
                                                className={`w-full py-1.5 rounded-lg text-center font-bold text-[12px] transition-all active:scale-95 ${
                                                    anom.failed 
                    ...
                    <div className="bg-slate-950 border border-slate-850 p-3 rounded-xl flex items-center justify-between text-[12px] text-slate-400">
                        <div className="flex flex-col gap-0.5">
                            <span className="uppercase font-bold text-slate-400">Model Præcision</span>
                            <span>LSTM model inference</span>
                        </div>
                        <span className="font-bold text-purple-400 font-mono text-sm">98.4% F1-SCORE</span>
                    </div>
                </div>
            </div>

            {/* Maintenance Policy Panel */}
            {totalPassengersTransported < 150 && (tutorialStep ?? 0) >= 6 ? (
                <div className="mt-6 bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center text-center shrink-0">
                    <Brain className="text-slate-400 mb-2 animate-pulse" size={32} />
                    ...
                    <div className="mt-3 bg-slate-950/60 border border-slate-850 px-4 py-2 rounded-xl text-sm font-bold text-slate-300 font-mono">
                ...
                <div className="mt-6 bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col gap-4 shrink-0">
                        ...
                        <div>
                            <h3 className="text-sm font-semibold text-slate-400 flex items-center gap-2">
                                ...
                                {tutorialStep !== undefined && tutorialStep < 3 && (
                                    <span className="text-[12px] bg-red-950 text-rose-400 px-2 py-0.5 rounded border border-rose-900/50 uppercase font-black animate-pulse">Låst under tutorial</span>
                                )}
                            </h3>
                            <p className="text-sm text-slate-400">Invester i Forskning & Udvikling for at opgradere din vedligeholdelses-strategi (Forsknings-hastighed: +{Math.round((1.0 + dataAnalystsCount * 0.3) * 100)}%).</p>
                        </div>
                        {activeResearch && (
                            <div className="flex flex-col items-end gap-1 text-[12px] text-slate-400 max-w-[200px] w-full">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        ...
                        <div className={`p-3 rounded-xl border flex flex-col gap-2 ${currentStrategy === 'REACTIVE' ? 'bg-red-950/20 border-red-500/50' : 'bg-slate-950/40 border-slate-850'}`}>
                            <div className="flex justify-between items-start">
                                <div>
                                    <span className="text-sm font-bold text-slate-200">1. Reaktiv</span>
                                    <div className="text-[9px] text-slate-400 uppercase mt-0.5 font-bold">Ingen løbende pris</div>
                                </div>
